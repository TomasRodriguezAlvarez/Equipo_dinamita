...
Independiente del entrenamiento (punto 2 de la pauta). Es la misma lógica que
`PyTorch/scriptsPython/dataset/preparar_dataset.py` — el dataset es compartido por
las dos ramas del proyecto (PyTorch y TensorFlow), así que basta con correr uno de
los dos scripts; este existe para que la carpeta tensorflow/scriptsPython/ sea
autocontenida y no dependa de PyTorch/.
...
def main():
    args = parse_args()
    separar(Path(args.origen), Path(args.destino), args.train, args.valid, args.test, args.seed)
